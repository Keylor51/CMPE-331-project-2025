...
document.addEventListener('DOMContentLoaded', () => {
    if (localStorage.getItem('jwt_token')) {
        const username = atob(localStorage.getItem('jwt_token')).split(':')[0];
...
async function handleLogin(e) {
    ...
    const securedUrl = `${API_BASE_URL}/roster/generate/DUMMYFLIGHT`; 

    try {
        const authString = btoa(`${username}:${password}`); 
        const response = await fetch(securedUrl, { method: 'GET', headers: { 'Authorization': `Basic ${authString}` } });
        ...
        alert("Could not connect to MainSystem API on localhost:8080."); 
    }
}
...
async function loadRoster(flightId) {
    ...
    try {
        ...
        } else { alert("Failed to load roster."); }
